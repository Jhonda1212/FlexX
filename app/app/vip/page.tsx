...
function roomTheme(room: VipRoom): RoomTheme {
  ...
  if (name.includes("dorada") || theme.includes("gold")) {
    return {
      ...
      card: "border-[var(--gold)]/28 bg-[radial-gradient(circle_at_78%_0%,rgba(212,175,55,0.18),transparent_34%),linear-gradient(145deg,rgba(212,175,55,0.1),rgba(255,255,255,0.035)_52%,rgba(0,0,0,0.24))]",
  ...
  if (name.includes("roja") || theme.includes("red")) {
    return {
      ...
      badges: ["Social"],
      card: "border-red-500/25 bg-[radial-gradient(circle_at_78%_0%,rgba(127,29,29,0.32),transparent_34%),linear-gradient(145deg,rgba(92,15,32,0.2),rgba(255,255,255,0.035)_52%,rgba(0,0,0,0.24))]",
  ...
  return {
    ...
    badges: ["Intima"],
    card: "border-white/12 bg-[radial-gradient(circle_at_78%_0%,rgba(229,231,235,0.12),transparent_34%),linear-gradient(145deg,rgba(0,0,0,0.58),rgba(255,255,255,0.035)_54%,rgba(31,41,55,0.16))]",
...
function RoomCard({ room }: { room: VipRoom }) {
  ...
  return (
    <FlexCard className={`group flex min-h-[440px] flex-col overflow-hidden rounded-2xl p-0 transition-[transform,border-color,box-shadow,background-color] duration-500 ease-out hover:-translate-y-0.5 hover:border-white/20 ${theme.glow} ${theme.card}`}>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <p className="mt-3 min-h-16 text-sm leading-6 text-white/72">
          {room.description ?? theme.fallbackDescription}
        </p>

        <div className="mt-5 grid gap-2 text-sm text-white/76">
          <div className="flex items-center justify-between rounded-full border border-white/10 bg-black/24 px-4 py-2.5">
            <span>Capacidad</span>
        ...
        <Link href={`/app/vip/${room.id}/share`} className="mt-auto pt-5">
    ...
    <div className="mx-auto max-w-6xl space-y-7">
      ...
      {loading ? (
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          <FlexSkeleton className="h-[440px] rounded-2xl" />
          <FlexSkeleton className="h-[440px] rounded-2xl" />
          <FlexSkeleton className="h-[440px] rounded-2xl" />
        </div>
      ) : null}
